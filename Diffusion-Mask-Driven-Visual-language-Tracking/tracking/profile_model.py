...
def evaluate_vit(model, text,template, search,search_mask):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search,text ,search_mask),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search,text,search_mask)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search,text ,search_mask)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "ostrack":
        ...
        search_mask = torch.randn((24, 24)).unsqueeze(dim=0).to(device)
        merge_layer = cfg.MODEL.BACKBONE.MERGE_LAYER
        if merge_layer <= 0:
            evaluate_vit(model, text, template, search, search_mask)
        else:
            evaluate_vit_separate(model, text, template, search,search_mask)
